...
def get_train_annotations_from_dataset(labels,
                                        train_dataset,
                                        train_image_folder, 
                                        valid_dataset = None,
                                        valid_image_folder = "", 
                                        is_only_detect = False):
    train_anns = parse_dataset_to_annotation(train_dataset, train_image_folder, labels, is_only_detect)
    if type(valid_dataset) != type(None):
        valid_anns = parse_dataset_to_annotation(valid_dataset,valid_image_folder,labels, is_only_detect)
    else:
        # TODO : check here splited at 0.8
...
def parse_dataset_to_annotation(dataset, image_dir, labels_naming=[], is_only_detect=False):
    if is_only_detect:
    ...
    for ann in dataset:
        image_file = os.path.join(image_dir, ann["id"] + "." + ann["ext"])
        
        if not os.path.exists(image_file):
            continue
        
        annotation = Annotation(image_file)
